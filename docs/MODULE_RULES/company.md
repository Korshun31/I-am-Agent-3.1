...
5. **Отклонить** → приглашение удаляется → админу уведомление "{email} отклонил приглашение" → модалка закрывается → пользователь
нажимает "Создать аккаунт" повторно → обычная регистрация с workspace
