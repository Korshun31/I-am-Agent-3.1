...
### Тарифные планы (`users_profile.plan`)

| Значение | Описание |
|----------|----------|
| `standard` | Базовый пользователь |
| `premium` | Владелец компании или участник команды |
| `korshun` | Super-admin (приватный тариф разработчика, назначается вручную) |

Поле `users_profile.role` — устаревшее (TD-001), не используется. Тариф читается только из `users_profile.plan`.
