...
- ⬜ TD-121 — унификация формул в `statisticsCalc.js`. Сейчас `computeTopProperties` и `computeAgentLeaderboard` суммируют `b.totalPrice` напрямую, а `computeRevenue` / `computeAgencyIncome` — через `monthlyBreakdown`. Это даёт расхождение: в одном периоде «топ объект» и «оборот по этому объекту» могут не совпасть. На одной валюте почти незаметно, но при включении конвертации (TD-120) станет видимым багом. Найдено tester-агентом 2026-05-05. Закрыть ДО старта TD-120.
