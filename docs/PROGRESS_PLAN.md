...
- ⏳ TD-114 — отложен 2026-05-03. Korshun решил: сначала сделать на вебе ОТДЕЛЬНУЮ страницу «Статистика» (не дашборд), потом мобильную переделать под неё, чтобы получился настоящий паритет. Расчётная утилита `src/utils/dashboardStats.js` уже вынесена и подключена к веб-дашборду (рефактор `2b47d36`) — основа готова, ждём дизайна страницы статистики.
