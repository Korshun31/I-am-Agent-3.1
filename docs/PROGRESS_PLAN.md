...
- ⏳ TD-114 — мобильный StatisticsScreen, паритет с вебом. Частично закрыто 2026-05-05: пикер периода, четыре KPI-карточки сеткой 2x2 (оборот, доход агентства, активные брони, заполняемость), график выручки/дохода за 12 месяцев + прогноз (горизонтальный скролл, цифры всегда видны, округление до тысяч, ширина колонки 80pt, ширина столбика 32pt), модалка breakdown по объектам с навигацией ‹/› (через `<Modal presentationStyle="pageSheet">`). Веб-компоненты получили опциональные props (`cardFlexBasis`, `colWidth`, `barMaxWidth`, `alwaysShowValues`, `fullNumbers`, `mobileMode`) для мобильного режима — веб не сломан. Осталось: график количества броней, топ-5 объектов, лидерборд агентов.
- ⏳ TD-120 — курсы валют для статистики. Согласовано 2026-05-05: таблица `currency_rates` в Supabase, обновление раз в сутки через Edge Function (источник ECB через api.frankfurter.app), архив курсов по датам. В `statisticsCalc.js` каждая бронь конвертируется в отчётную валюту пользователя по курсу даты — для прошлых броней по `checkIn`, для будущих по `createdAt`. Конвертация только в статистике, ничего больше в приложении не трогаем. План реализации в 4 фазы: миграция БД → Edge Function → клиентский слой за feature-flag → включение в UI сначала на вебе, потом на мобайле.
  ...
  - ⏳ Фаза D (включение конвертации в UI) готова в коде 2026-05-05. `WebStatisticsScreen.js` и `StatisticsScreen.js` подключают `useCurrencyRates`, собирают `propertyCurrencyById` и `fxCtx = { rates, targetCurrency, propertyCurrencyById, today }` через `useMemo`, передают `fxCtx` во все денежные расчёты (`computeRevenue`, `computeAgencyIncome`, `computeMonthlyRevenue`, `computeMonthlyForecast`, `computeTopProperties`, `computeAgentLeaderboard`, `breakdownByPropertyForMonth`). Заодно на вебе вынесен `breakdownRows` в `useMemo` (раньше было inline в JSX) — теперь симметрично с мобайлом. Реальная конвертация заработает только после ручной накатки миграций и заполнения таблицы курсов в Supabase. До этого `convertAmount` graceful degrade — числа на экране как раньше. Архитектор подтвердил отсутствие регрессий, тестер дал чеклист на 18 пунктов (9 проверяемых до накатки, 9 после).
  - ✅ Фаза C (клиентский слой курсов, без UI-интеграции) закрыта 2026-05-05. Коммит 1 (`a2ae6f9`): новый `src/services/currencyRatesService.js` (читает `currency_rates` за последние 90 дней), `src/utils/currencyConvert.js` (центральные хелперы — `indexRates` строит `Map` для O(log n) бинарного поиска, `bookingFxDate` = checkIn если бронь наступила иначе createdAt, `bookingCurrency` = currency брони → property → отчётная, `convertAmount` — главная точка входа с graceful degrade при отсутствии курса), `src/context/CurrencyRatesContext.js` (provider с защитой от гонки кэша как в фазе A: один `useEffect`, два этапа в одной async — кэш TTL 24ч для мгновенного paint, потом всегда fetch и обновление). `App.js` обёрнут в `<CurrencyRatesProvider>` сразу под `<UserProvider>` — курсы грузятся при старте, до login. Миграция `20260505000000_currency_rates.sql` поправлена: SELECT-политика расширена до `anon, authenticated` (курсы публичные ECB-данные, безопасно открыть для подгрузки до логина). Коммит 2: `src/utils/statisticsCalc.js` получил опциональный `ctx={rates,targetCurrency,propertyCurrencyById,today}` во всех экспортах через локальный хелпер `fx(amount, b, ctx) => convertAmount(amount, b, ctx?.today, ctx)`. Рекурсивный проброс `ctx` через `buildMonthEntry` для `computeMonthlyRevenue`/`computeMonthlyForecast`. Без `ctx` поведение 100% обратно совместимо. UI пока ничего не показывает по-новому — экраны статистики не передают `ctx`. Дальше фаза D — фактическая активация в UI.
  - ⏳ Фаза B (инфраструктура курсов в Supabase, без UI). Код готов 2026-05-05, не накатан. Файлы: `supabase/migrations/20260505000000_currency_rates.sql` (таблица `currency_rates` PK=(rate_date, base_currency, quote_currency), RLS read для authenticated, write только service_role); `supabase/functions/sync-currency-rates/index.ts` (Edge Function через `jsr:@supabase/supabase-js@2`, фетчит `api.frankfurter.app/latest?from=USD&to=EUR,THB,RUB`, делает upsert; защита: только POST + проверка `Authorization: Bearer <service_role_key>`); `supabase/migrations/20260505000001_currency_rates_cron.sql` (pg_cron job ежедневно в 06:00 UTC, плейсхолдер `<PROJECT_REF>` пользователь подставляет вручную, ключ через `current_setting('app.settings.service_role_key', true)`). Архитектор-агент аудит провёл: блокер с синтаксисом `cron.unschedule WHERE EXISTS` исправлен на `DO $$ ... PERFORM ... $$`. Импорт supabase-js приведён к `jsr:` (как в `invite-agent`). Минорные открытые пункты: secret через GUC а не Vault (приемлемо), pg_net.http_post таймаут 5000мс по умолчанию (мониторить через `net._http_response`). Тестер дал smoke-чеклист на 10 пунктов для прогона после деплоя. Шаги пользователя: (1) включить расширения `pg_cron` и `pg_net` в Dashboard → Database → Extensions; (2) `ALTER DATABASE postgres SET app.settings.service_role_key = '<JWT>'`; (3) заменить `<PROJECT_REF>` в файле миграции `20260505000001`; (4) `supabase db push`; (5) `supabase functions deploy sync-currency-rates`; (6) проверочный curl POST → ожидать `{"ok":true,"upserted":4}`. До фазы C нужно закрыть TD-121 (унификация формул).
