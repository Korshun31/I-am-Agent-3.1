...
- ⬜ Backup prod базы перед миграцией. Сразу после успешного backup — `DROP TABLE IF EXISTS properties_backup_before_updated_at;` (страховочная копия от 2026-04-22, попадёт в snapshot, можно дропать).
