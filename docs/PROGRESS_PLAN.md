...
>
> Последнее обновление: 2026-04-27 (после аудита кода и git log)
...
TD-072 (занятые даты в picker), TD-074 (сжатие фото), TD-076 (commission reminders), TD-077 (отмена reminders), TD-082 (monthly_breakdown), TD-087 (can_delete_booking UI+server), TD-089 (mobile %/сумма переключатель).
...
TD-022 (mobile UI «Команда»), TD-021 (mobile invite accept экран), TD-114 (StatisticsScreen parity), TD-046 (currency на объекте), TD-059 (auto-approve rejected), TD-111 (markActionTaken), TD-113 (realtime бейдж), TD-041 (OAuth pending invitations).
Зачем после web: mobile фичи — крупные новые экраны, дороже web-патчей.

### Фаза 9 — Технический долг кода (5-7 дней)
TD-020 (UserContext), TD-035 (RPC `get_full_user_profile`), TD-043 (resort_id → parent_id, 78 вхождений — рискованный), TD-052 (мёртвый код в триггере), TD-001 (users_profile.role мусор), TD-034 (signUp settings merge), TD-036 (объединить OAuth providers), TD-037 (выйти со всех устройств), TD-030 (audit log), TD-045 (videos массив), TD-047 (address поле), TD-055 (было/стало черновик), TD-056 (унифицировать needsApproval), TD-060 (каскадное обновление района), TD-004/TD-010/TD-012 (i18n).
...
P1-001/TD-009 (роль помощника админа), P1-002 (CHECK rejection_reason), P1-004 (downgrade тарифа), P1-005 (чистка термина «agent»), P2-001/P2-002/P2-004, P3-001/P3-002/P3-004, TD-090/P3-003 (Web Push), TD-108 (web reminders календарных), TD-110 (перенос события), TD-016 (disposable emails) + 10 финальных шагов релиза.
...
- ✅ TD-057 — `can_edit_info` + `can_edit_prices` toggles (есть в WebTeamSection)

## Properties

- ⬜ TD-005 — TODO permissions guards в UI (= P2-003)
...
- ⬜ TD-009 — `can_moderate_properties` для старшего агента (= P1-001)
...
- ⏳ TD-052 — `auto_set_property_company` мёртвый код (триггер всё ещё проверяет 'submitted')
- ✅ TD-053 — фото в Storage не удаляются при delete (коммит `c048ded`)
- ⬜ TD-054 — UNIQUE constraint на `code`
- ⬜ TD-055 — визуальное «было/стало» при просмотре черновика
- ⬜ TD-056 — `needsApproval` разная логика веб vs мобильный
- ✅ TD-058 — раздельное сохранение по permissions (коммиты `63258ea`, `475a89a`)
- ⬜ TD-059 — мобильный авто-approve rejected при правке админом
...
- ✅ TD-086 — `booking_agent_id` (миграция `20260415000006`)
- ⬜ TD-087 — `can_delete_booking` UI и server check
...
- ⬜ TD-099 — RLS contacts по `booking_agent_id` (зависит от TD-086, который уже закрыт — можно делать)
...
- ⬜ TD-111 — мобильный `markActionTaken` после approve/reject
- ✅ TD-112 — Веб лишние кнопки модерации (коммит `fb5d79b`)
...
- ⬜ P1-001 — Роль «помощник админа» / `can_moderate_properties` (= TD-009)
...
- ⬜ P2-003 — Permissions UI guards (= TD-005)
