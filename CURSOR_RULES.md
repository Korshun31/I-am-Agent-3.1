...
TD-059: Мобильный PropertyDetailScreen — нет авто-одобрения rejected объекта при редактировании админом
...
TD-088: Удалить can_see_financials из проекта — агент всегда видит финансы своих бронирований
...
TD-102: can_manage_clients — удалить из проекта, агент всегда работает со своими контактами
...
TD-111: Мобильный PropertyNotificationsModal — markActionTaken не вызывается после одобрения/отклонения объекта
Что сделать: после approve/reject вызывать markActionTaken(notificationId) чтобы уведомление помечалось обработанным

TD-112: Веб WebNotificationBell — кнопки одобрить/отклонить в панели уведомлений лишние
...
Что сделать: ALTER TABLE contacts ADD CHECK (trim(name) <> '')
