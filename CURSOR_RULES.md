...
TD-028: deactivate_member (DB function) использует DELETE вместо soft-delete
Что сделать: заменить DELETE на UPDATE status='inactive' в company_members (колонка status уже существует)
