      ...
      <ScrollView style={styles.scrollArea} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={s.root}>
          <View style={s.headerRow}>
            <Text style={s.title}>👥 {t('teamMembers')}</Text>
            {!showInviteForm && (
          ...
          {!loading && archivedMembers.length > 0 && (
            <View style={s.section}>
          ...
          {!loading && activeMembers.length === 0 && invitations.length === 0 && !showInviteForm && (
...
const styles = StyleSheet.create({
  ...
  fixedTop: { paddingTop: TOP_INSET, paddingHorizontal: 20, paddingBottom: 0 },
  header: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 },
  backBtn: { width: 52, padding: 8, alignItems: 'flex-start', justifyContent: 'center' },
  backArrowText: { fontSize: 24, color: COLORS.backArrow },
  headerTitle: { flex: 1, fontSize: 22, fontWeight: '700', color: COLORS.title, textAlign: 'center' },
